...
export async function getHotelsWithAvailability(req: AuthenticatedRequest, res: Response) {
  const { userId } = req;

  const hotels = await hotelsService.getHotelsWithAvailability(userId);
  res.status(httpStatus.OK).send(hotels);
}
